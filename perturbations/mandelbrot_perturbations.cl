...
inline double sq_mod(cdouble_t c){
    return c.real*c.real + c.imag*c.imag;
}

inline cdouble_t get_init_delta(
    const int ref_x,
    const int ref_y,
    const double w_per_pix,
    const double h_per_pix,
    const int x,
    const int y
){
    return cdouble_new((x - ref_x)* w_per_pix, (ref_y - y) * h_per_pix);
}

inline cdouble_t get_delta_estimate(
    const int num_terms,
    ...
    for (int k=0; k < num_terms; k++){
        out = cdouble_add(out, cdouble_mul(terms[term_index_offset + k], scaled_delta));
        scaled_delta = cdouble_mul(scaled_delta, init_scaled_delta);
    }
    return out;
 }

 inline cdouble_t get_reference(__constant cdouble_t* precise_reference, int i) {
...
__kernel void approximate_pixels(
    ...
    const cdouble_t delta_0 = get_init_delta(ref_x, ref_y, w_per_pix, h_per_pix, x, y);
    cdouble_t delta_i = get_delta_estimate(num_terms, terms, term_scaling_factor, delta_0, iter_accurate);
    cdouble_t ref_i, point = cdouble_new(0, 0);
    double point_sq_mod;
    int i = iter_accurate;
    while (i <= breakout) {
        ref_i = get_reference(precise_reference, i);
        point = cdouble_add(delta_i, ref_i);
        ...
        delta_i = cdouble_add(
            cdouble_rmul(2, cdouble_mul(get_reference(precise_reference, i), delta_i)),
            cdouble_add(
